...
mongoose.connect('mongodb+srv://khurram:khurramaftab@cluster0.7lsze.mongodb.net/crossoverdb', { useNewUrlParser: true });
...
let func = async () => {
 ...
 let d = new Date();
        ...
        if (json2.length == 0) {
          ...
          if (price > json1[0].close) {
            ...
            if (yesterdayaverage < yesterdayclose) {
              ...
              // const ticker = { date:json1[0].date, ticker: data[iterator], average: price, closed_price: json1[0].close , Volume:json1[0].volume}
              //   console.log(ticker);
              //   increased_tickers.push(ticker);
              //   const db= await TickerSchema.create(ticker);
            }
            ...
            if (price < json1[0].close) {
              if (yesterdayaverage > yesterdayclose) {
                ...
                //   ticker = { date:json1[0].date, ticker: data[iterator], average: price, closed_price: json1[0].close, Volume:json1[0].volume }
                //   console.log(ticker);
                //   increased_tickers.push(ticker);
                //  const db= await TickerSchema.create(ticker);
              }
          ...
          //  var fbenddate = document.getElementById("myData").innerHTML=json.endDate;
          //  var fbstartdate = document.getElementById("myDataa").innerHTML=json.startDate;
          //  var ticker = document.getElementById("ticker").innerHTML=json.ticker;
          //  div.innerHTML = 'Name: ' + json.endDate 
          //  if (json1[0].close < sum1) {
          //             console.log("close greater than sum")
          //             const ticker = { date: todayDate, ticker: data[iterator], average: price, closed_price: json1[0].close }
          //             increased_tickers.push(ticker);
          //             TickerSchema.create(ticker, function (err, db) {
          //               if (!err) {
  ...
  // var mailOptions = {
  //   from: '"Tickers ?" <no-reply@tickers.com>', // sender address
  //   to: 'huzaifagladiator@gmail.com', // list of receivers( , separated here)
  //   subject: 'Here is the daily report of tickers increased', // Subject line
  //   html: email_html // html body
